...
export function check_account(id, password) {
  ...
  return dispatch => {
    var url = process.env.USER_URL + "login"
    fetch(url, {
      method: 'POST',
      headers: {
...
export function upload_image(file, post, index, paragraph_index) {
  return dispatch => {
    var url = process.env.FILE_URL + 'upload'
...
export function create_new_post() {
  return dispatch => {
    var url = process.env.POST_URL + "add/"
    fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
...
export function save_local_changes(post) {
  var url = process.env.POST_URL + "update/" + post._id.toString()
  return dispatch => {
    fetch(url, {
      method: 'POST',
      headers: {
...
export function delete_post(post_id) {
  var url = process.env.POST_URL + "delete/" +post_id.toString()
  return dispatch => {
    fetch(url, {
      method: 'GET',
      headers: {
